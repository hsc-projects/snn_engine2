...
class SynapseRepresentation(GPUArrayCollection):
    ...
    def __init__(self,
                 ...
                 N_pos,
                 N_flags: NeuronFlags,
                 G_flags: LocationGroupFlags,
                 G_props: LocationGroupProperties,
                 g2g_info_arrays: G2GInfoArrays,
                 device: int,
        ...
        self.G_group_delay_counts = G_group_delay_counts

        self.N_pos = N_pos
        self.N_flags = N_flags
        self.G_flags = G_flags
        self.G_props = G_props
        self.g2g_info_arrays = g2g_info_arrays
        ...
        self.L_winner_take_all_map[self._config.max_winner_take_all_layer_size, :] = 0
        self.n_wta_layers = 0
        ...
        (self.G_conn_probs,
         ...
         self.G_exp_exc_ccsyn_per_snk_type_and_delay) = self._fill_syn_counts(
            ...
            type_group_conns=type_group_conns,
            shapes=shapes,
            G_neuron_counts=G_neuron_counts)
        ...
        self.init_N_weights(type_group_conns)
    ...
    def _G_swap_tensor(self):
        max_neurons_per_group = self.group_counts(self.G_neuron_counts).max().item()
    ...
    def init_N_rep_and_N_delays(
        ...
        for i, gc in enumerate(type_group_conns):
            ...
            snn_construction_gpu.fill_N_rep(
                ...
                G_rep=self.g2g_info_arrays.G_rep.data_ptr(),
    ...
    def _N_relative_G_indices(self):
        all_groups = self.N_flags.group.type(torch.int64)
        ...
        inh_neurons = self.N_flags.type == NeuronTypes.INHIBITORY.value
        ...
        return (self.N_flags.id - start_indices).type(torch.int32)
    ...
    def _single_group_swap(self,
        ...
        neurons = self.N_flags.select_ids_by_groups(groups=program[1]).type(torch.int64)
        n_neurons = len(neurons)

        assert (neuron_group_indices[neuron_group_indices >= 0]
                + program[1, 0] - self.N_flags.group[neurons]).sum() == 0
        ...
        self.RepBackend.swap_groups(
            ...
            self.G_neuron_typed_ccount.data_ptr(), neuron_group_counts.data_ptr(),
            print_idx)
        # noinspection PyUnusedLocal
    ...
    def add_winner_take_all_layer(self, neurons):
        assert self.n_wta_layers < self._config.max_n_winner_take_all_layers
        assert len(neurons) <= self._config.max_winner_take_all_layer_size
        if isinstance(neurons, list):
            neurons = torch.Tensor([neurons]).to(self.device)

        self.L_winner_take_all_map[:len(neurons), self.n_wta_layers] = neurons
        self.L_winner_take_all_map[self._config.max_winner_take_all_layer_size, self.n_wta_layers] = len(neurons)
    ...
    def make_sensory_groups(
        ...
        for group in groups:
            ...
            n_sensory_neurons = max(int(G_neuron_counts[ntype - 1, :][group]/self._config.S), 1)

            neuron_group_ids_exc = self.N_flags.select_ids_by_groups([group], ntype=NeuronTypes.EXCITATORY.value)
            ...
            filter_neuron_ids_inh = self.N_flags.select_ids_by_groups([group], ntype=NeuronTypes.INHIBITORY.value)

            self.N_flags.b_sensory_input[sensory_neuron_ids] = 1
    ...
    def swap_validation(self, j, neurons):
        ...
        b[3] = self.N_flags.group[b[2]].cpu().numpy()
